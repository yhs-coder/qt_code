...
void PaintWidget::paintLow()
{
    ...
    int pointX[7] = {0};
    for (int i = 0; i < 7; i++) {
        pointX[i] = m_lblLow->x() + PADDING + (m_lblLow->width() - PADDING * 2) / 6 * i;
    }
    ...
    int pointY[7] = {0};
    // 将标签分为一半
    int yCenter   = m_lblLow->height() / 2;
    int increment = m_lblLow->height() / 20;

    for (int i = 0; i < 7; i++) {
        // 在窗口中，y轴是向下增长
        // 当温度比平均温度高，温度向上增长，但y轴向上减少
        pointY[i] = yCenter + (-(m_lowTemp[i] - tempAverage) * increment);
    }
    ...
    for (int i = 0; i < 7; i++) {
        painter.drawEllipse(QPoint(pointX[i], pointY[i]), POINT_RADIUS, POINT_RADIUS);
        painter.drawText(QPoint(pointX[i] - TEXT_OFFSET_X, pointY[i] - TEXT_OFFSET_Y), QString::number(m_lowTemp[i]) + "°");
    }
